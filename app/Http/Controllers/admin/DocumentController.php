...
class DocumentController extends Controller
{
    ...
    public function createReturn(Request $request)
    {
        ...
        //check if request type 0 ['sell'] to set the new type to 3 ['sell return']
        //if document type != 0 that mean it ['buy'] so we set new type to 1 ['buy retrun']
    ...
    private function valiadateBranch($request){
        // validate that branch is required
        // set branch to use branch if there is one
        // return error if branch is required and not provided
        if ($request->user()->branch_id) {
    ...
    public function update(Request $request, $doc)
    {
        ...
        // set branch to use branch if there is one
        // return error if branch is required and not provided
        if ($document->type === 6 && !$request->branch_to) {
            return response()->json('branch to is required', 400);
        }
    ...
    public function close($id)
    {
        ...
        foreach ($items as $item) {
            ...
            //check if doc type is inventory  or first balance 
            if ($document->type >= 4 && $document->type <= 5) {
                $rec['in'] = $item->qty;
                defineItemStock($rec);
            } else{
                addItemStock($rec);
